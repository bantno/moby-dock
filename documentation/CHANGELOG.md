...
## 2026-06-30 — Jack — Merge `origin/main` into `corbin-dev` (stall model + sensor/QP hardening)

**Branch/commit:** corbin-dev (merge of `origin/main` `3c77a46`)
**What changed:** Synced `corbin-dev` with `origin/main`, which had advanced via PRs #3–#4 to carry
Brian's **NACA 4414 viscous-stall plant model** (`stall-model`, see his entry below), the
**altitude-sensor model + descent-barrier / QP-fallback hardening** (`acdc27b`), and impact-barrier
realism TODOs (`35bf0ce`). Real 3-way merge from base `8e2770a`. **All code, data, and configs merged
cleanly** — the only conflict was the append-only `CHANGELOG.md`, resolved by **keeping both sides'
entries** (Brian's 06-30 stall entry on top, Jack's 06-27…06-29 entries below); `TODO.md` auto-merged.
**Why:** Keep the working branch current with `main` and bring the stall plant + sensor/QP hardening
onto `corbin-dev`.
**Verified:** clean configure + Release build (no errors); **45/45 tests pass** (`ctest -C Release`) on
the merged tree. Notably the merge *resolves* the `#19` failure Brian's branch reported (missing
`AHAB_sweep.stab`) — `corbin-dev`'s deck consolidation already repointed it, so the two lines are
semantically compatible.
**Follow-ups / notes for collaborator:** merge not yet pushed (left local for review). The
stall-model `CL_max`≈1.44 could be linked to the descent barrier's `CL_max` knob (Brian's follow-up).
**Files touched (merge):** conflict resolution in `documentation/CHANGELOG.md`; all other files merged
without conflict.
