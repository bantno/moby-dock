...
LonCtrlVec LonCBFFilter::filter(const LonCtrlVec& U_nom, const LonStateVec& X,
  ...
  if (cfg_.thrust_limits) {
    ...
    pushHocbf(thrustMaxRow(X, cfg_.Tmax, cfg_.c_thrust_max[0], cfg_.c_thrust_max[1]), true, 0.0);
  }
  ...
  auto solveWith = [&](bool best_effort) -> QPResult {
    ...
    // no per-control weights).
    P(LDE, LDE) = 1.0;
    P(LTDDOT, LTDDOT) = 1.0;
    q[LDE] = -U_nom[LDE];
    q[LTDDOT] = -U_nom[LTDDOT];
    // Soft constraints: per-constraint QUADRATIC slack penalty 1/2 w_i delta_i^2
    // (P-term; q stays 0 on slacks). Larger w_i => firmer constraint. Quadratic
    // (not linear w_i*delta_i): a pure-linear penalty makes P singular on the
    // slacks and OSQP then fails to converge on many feasible steps, firing the
    // best-effort fallback spuriously. In the best-effort solve, hard rows get a
    // slack at the dominant kBestEffortHardPenalty so it MINIMIZES their violation.
    for (int i = 0; i < nb; ++i)
      if (slack_col[i] >= 0)
